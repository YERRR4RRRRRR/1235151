...
try:
    ...
    from exUE5.spawnable_diagnostics import diagnose_camera_export_issue
except ModuleNotFoundError:
    from exporter_core import load_config, build_output_path
    from spawnable_diagnostics import diagnose_camera_export_issue
...
def _run_export():
    ...
    if config.get("use_selection_dialog", False):
        ...
        if sequence:
            ...
            display_name_fn = lambda item: getattr(item, "get_name", lambda: str(item))() if hasattr(item, "get_name") else str(item)
